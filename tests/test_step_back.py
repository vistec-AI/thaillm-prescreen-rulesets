...
class TestStepBackUrgencyReset:
    """Verify that __urgency flag is properly cleared on step_back
    so a re-answered OLDCARTS path doesn't inherit a stale flag."""

    @pytest.mark.asyncio
    async def test_urgency_flag_cleared_on_step_back_in_phase4(
        self, engine, mock_db, mock_repo,
    ):
        """Step back within phase 4 clears the __urgency flag from responses."""
        step = await _advance_to_phase(engine, mock_db, target_phase=4)
        if not isinstance(step, QuestionsStep):
            pytest.skip("OLDCARTS tree auto-resolved")

        # Answer a couple questions using _pick_answer_last (avoids urgency)
        answered_qids, step = await _answer_sequential_questions(
            engine, mock_db, step, count=3, picker=_pick_answer_last,
        )
        if len(answered_qids) < 2:
            pytest.skip("Not enough sequential questions")
        if isinstance(step, TerminationStep):
            pytest.skip("Session terminated during OLDCARTS")

        # Inject __urgency flag to simulate an UrgencyAction having fired
        row = mock_repo._sessions[("u1", "s1")]
        row.responses = {**row.responses, "__urgency": {"departments": ["dept006"]}}
        assert "__urgency" in row.responses, "Precondition: urgency flag set"

        # Step back — should clear __urgency
        step = await engine.step_back(mock_db, user_id="u1", session_id="s1")
        assert isinstance(step, QuestionsStep), "Expected QuestionsStep"

        row = mock_repo._sessions[("u1", "s1")]
        assert "__urgency" not in row.responses, (
            "__urgency flag should be cleared after step_back in phase 4"
        )

    @pytest.mark.asyncio
    async def test_urgency_flag_cleared_on_step_back_to_earlier_phase(
        self, engine, mock_db, mock_repo,
    ):
        """Stepping back from phase 5 to phase 4 also clears __urgency."""
        step = await _advance_to_phase(
            engine, mock_db, target_phase=5, picker=_pick_answer_last,
        )
        if not isinstance(step, QuestionsStep) or step.phase != 5:
            pytest.skip("Could not reach phase 5")

        # Inject __urgency flag (defensive — shouldn't exist at phase 5,
        # but verifying step-back clears it regardless)
        row = mock_repo._sessions[("u1", "s1")]
        row.responses = {**row.responses, "__urgency": {"departments": []}}

        # Step back from phase 5 → phase 4
        step = await engine.step_back(mock_db, user_id="u1", session_id="s1")

        row = mock_repo._sessions[("u1", "s1")]
        assert "__urgency" not in row.responses, (
            "__urgency flag should be cleared when reverting to phase 4"
        )

    @pytest.mark.asyncio
    async def test_no_spurious_urgency_termination_after_step_back(
        self, engine, mock_db, mock_repo,
    ):
        """After clearing urgency via step-back, completing OLDCARTS
        via OPD action should advance to phase 5 (not terminate)."""
        step = await _advance_to_phase(engine, mock_db, target_phase=4)
        if not isinstance(step, QuestionsStep):
            pytest.skip("OLDCARTS tree auto-resolved")

        # Answer several questions to stay in OLDCARTS
        answered_qids, step = await _answer_sequential_questions(
            engine, mock_db, step, count=3, picker=_pick_answer_last,
        )
        if len(answered_qids) < 2:
        ...
        # Inject __urgency flag
        row = mock_repo._sessions[("u1", "s1")]
        row.responses = {**row.responses, "__urgency": {"departments": ["dept006"]}}

        # Step back to clear the flag
        step = await engine.step_back(mock_db, user_id="u1", session_id="s1")
        assert isinstance(step, QuestionsStep), "Expected QuestionsStep"

        # Now continue answering through OLDCARTS using _pick_answer_last
        # until we exit phase 4 — should reach phase 5, not terminate
        ...
        # Should reach phase 5 (Past History), not terminate with urgency
        assert isinstance(step, QuestionsStep), (
            "Expected QuestionsStep (phase 5), got TerminationStep — "
            "stale __urgency flag may have caused spurious termination"
        )
        assert step.phase == 5, (
            f"Expected phase 5 (Past History) after OLDCARTS, got phase {step.phase}"
        )
