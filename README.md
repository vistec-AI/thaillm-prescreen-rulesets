...
1. **Department routing** — redirect the patient to the appropriate hospital department (13 departments)
...
### Prescreen Flow (8 phases)

| Phase | Name | File(s) | Description |
|-------|------|---------|-------------|
| 0 | Demographic inquiry | [`demographic.yaml`](./v1/rules/demographic.yaml) | Collect 14 demographic fields (age, gender, underlying diseases, medication, allergies, surgical history, conditional pregnancy block). Fields have conditional visibility. |
| 1 | ER critical symptom screen | [`er_symptom.yaml`](./v1/rules/er/er_symptom.yaml) | 20 life-threatening yes/no checks (some conditional on demographics). If any is true, early-terminate with Emergency severity. |
...
| 5 | Past History | [`past_history.yaml`](./v1/rules/past_history.yaml) | Height, weight, other medical conditions, and pediatric-specific fields (vaccination, developmental milestones). |
| 6 | Personal History | [`personal_history.yaml`](./v1/rules/personal_history.yaml) | Occupation, hometown, smoking history, and alcohol history. |
| 7 | OPD | [`opd.yaml`](./v1/rules/opd.yaml) | Conditional decision tree that uses answers from phase 4 to determine the final department assignment. |
...
└── rules/
    ...
    ├── past_history.yaml           # Phase 5: past history fields
    ├── personal_history.yaml       # Phase 6: personal history fields
    └── opd.yaml                    # Phase 7: OPD routing decision trees
...
Run a full end-to-end simulation of the `PrescreenPipeline` with mocked DB and LLM components. The script drives through all 8 rule-based phases plus LLM questioning and prediction, printing rich audit logs of every question and answer.
